...
const debug = require('debug')('wall:state_manager');
const _ = require('underscore');
const StateSchedule = require('server/state/state_schedule');
...
class StateManager {
  constructor(network) {
    ...
    this.network_.on('connection', function StateManagerClientHandler(socket) {
      socket.on('newclientstatecreated', (data) => {
        if (!(data.name in self.trackedState_) ||
          ...
          debug(`Did not create new state ${data.name} ${self.stateOwners_[data.name]} already owns it.`);
        }
      ...
      socket.on('newclientstateset', (data) => {
        if (self.stateOwners_[data.name] === socket.id) {
          console.assert(data.name in self.trackedState_);
          self.get(data.name).set(data.value, data.time);
        } else {
          debug(`State update for ${data.name} not accepted. State already owned by ${self.stateOwners_[data.name]}`);
        }
      });
  ...
  send() {
    ...
    var stateData = _.map(this.trackedState_, (state, name) => {
      return {
        name: name,
