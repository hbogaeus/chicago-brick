...
(function(factory) {
  ...
  const SharedState = function(name, interpolator, owner) {
  ...
  const NumberLerpInterpolator = (time, at, av, bt, bv) => {
    if (av === null || bv === null) {
      return null;
    }
  ...
  const ValueNearestInterpolator = (time, at, av, bt, bv) => {
  ...
  const CurrentValueInterpolator = (time, at, av, bt, bv) => time >= bt ? bv : av;

  const ObjectInterpolatorGenerator = (def) => {
    let dynamicInterpolator = {};
    for (var k in def) {
    ...
    return function ObjectInterpolator(time, at, av, bt, bv) {
      ...
      for (var k in av) {
        if (k in bv) {
          // interpolate
          let interpolator = dynamicInterpolator[k] || dynamicInterpolator['*'];
          if (!interpolator) {
  ...
  const ArrayInterpolatorGenerator = (def) => {
    // Def is an array of 1 generator reference.
    let dynamicInterpolator = decodeInterpolator(def[0]);

    return function ArrayInterpolator(time, at, av, bt, bv) {
      av = av || [];
      bv = bv || [];

      return av.map((value, index) =>  dynamicInterpolator(time, at, av[index], bt, bv[index]));

    };
  };

  const simpleInterpolators = [
    NumberLerpInterpolator,
    ValueNearestInterpolator,
    CurrentValueInterpolator
  ];

  const decodeInterpolator = (def) => {
