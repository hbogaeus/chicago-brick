...
(function(factory) {
  ...
  const geo = {
    ...
    intersectPolygonLine: (polygon, ax, ay, bx, by) => {
      for (var i=1; i < polygon.points.length; i++) {
        let m = polygon.points[i-1];
        let n = polygon.points[i];
        let intersection = geo.intersection(ax, ay, bx, by, m.x, m.y, n.x, n.y);
        if (intersection) {
    ...
    intersectPolygonPolygon: (testPolygon, againstPolygon) => {
      for (var i=1; i < testPolygon.points.length; i++) {
        let m = testPolygon.points[i-1];
        let n = testPolygon.points[i];
        let intersection = geo.intersectPolygonLine(againstPolygon, m.x, m.y, n.x, n.y);
        if (intersection) {
          return intersection;
        }
      }
    ...
    cutPolygon: (polygon, ax, ay, bx, by) => {
      ...
      for (var i = 0; i < polygon.points.length; ++i) {
        ...
        let det1 = geo.side(ax, ay, bx, by, p1.x, p1.y);
        let points = det1 > 0 ? leftPoints : rightPoints;
        let otherPoints = points === leftPoints ? rightPoints : leftPoints;
        ...
        if (det1 == 0) {
          ...
          let p0 = polygon.points[(i - 1 + polygon.points.length) % polygon.points.length];
          let det0 = geo.side(ax, ay, bx, by, p0.x, p0.y);
          if (det0 == 0) {
            // Need to add it:
        ...
        let det2 = geo.side(ax, ay, bx, by, p2.x, p2.y);
        ...
        let point = geo.intersection(ax, ay, bx, by, p1.x, p1.y, p2.x, p2.y);
    ...
    distanceToPolygon: (polygon, x, y) => {
      ...
      for (var i = 0; i < polygon.points.length - 1; i++) {
        let p1 = polygon.points[i];
        let p2 = polygon.points[i+1];
        let d = geo.distanceToLine(p1.x, p1.y, p2.x, p2.y, x, y);
